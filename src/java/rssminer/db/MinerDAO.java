...
public class MinerDAO {
    ...
    private List<Feed> fetchFeeds(int userID, List<Integer> feedids) throws SQLException {
    ...
    private Feed createFeed(ResultSet rs, boolean withSummary) throws SQLException {
        Feed f = new Feed();
        f.setId(rs.getInt(1));
    ...
    public static List<Feed> removeDuplicate(List<Feed> feeds) {
        ...
        for (Feed f : feeds) {
            boolean dup = false;
            for (Feed e : results) {
                if (e.getLink().equals(f.getLink()) || e.getTitle().equals(f.getTitle())
    ...
    // for folder
    public List<Feed> fetchFolderOldest(int userID, List<Integer> rssIDs, int limit, int offset)
            throws SQLException {
        StringBuilder sb = createBuilder(rssIDs);
        sb.append(FEED_FIELD).append(userID);
        sb.append(" WHERE f.rss_link_id in ");
    ...
    public List<Feed> fetchFolderRead(int userID, List<Integer> rssIDs, int limit, int offset)
            throws SQLException {
        StringBuilder sb = createBuilder(rssIDs);
        sb.append(FEED_FIELD).append(userID);
    ...
    public List<Feed> fetchFolderVote(int userID, List<Integer> rssIDs, int limit, int offset)
            throws SQLException {
        StringBuilder sb = createBuilder(rssIDs);
        sb.append(FEED_FIELD).append(userID);
    ...
    public List<Feed> fetchGNewest(int userID, int limit, int offset) throws SQLException {
        ...
        // only recent 30 days
        sb.append((int) (System.currentTimeMillis() / 1000) - 3600 * 24 * 30);
        sb.append(" order by f.published_ts desc ");
        appendLimitOffset(sb, limit, offset);
        return fetchFeedsWithScore(userID, sb.toString());
    }

    public List<Feed> fetchGRead(int userID, int limit, int offset) throws SQLException {
        StringBuilder sb = createBuilder(null);
        sb.append(FEED_FIELD).append(userID);
    ...
    public List<Feed> fetchGVote(int userID, int limit, int offset) throws SQLException {
    ...
    public List<Feed> fetchSubOldest(int userID, int subID, int limit, int offset)
            throws SQLException {
        StringBuilder sb = createBuilder(null);
        sb.append(FEED_FIELD).append(userID);
        sb.append(" WHERE f.rss_link_id = ").append(subID);
    ...
    private List<Subscription> fetchSubs(String select, int userID) throws SQLException {
        Connection con = ds.getConnection();
        try {
            Statement stat = con.createStatement();
