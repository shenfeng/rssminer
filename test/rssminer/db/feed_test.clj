...
(deftest test-save-feeds
  ...
  (save-feeds {:entries [{:link link :author "three"}]} 2) ; diff rss-link-id
...
(deftest test-save-feeds2               ; same url
  (with-mysql (delete-rows :feeds ["id > 0"]))
  (save-feeds {:entries [{:link link :author "one" :simhash 7}
                         {:link link :author "two" :simhash 2}]} 1)
  (let [feeds (mysql-query ["select * from feeds"])]
    (is (seq (filter (fn [f] (= "one" (:author f))) feeds)))
    (is (= 1 (count feeds)))))

(deftest test-save-feeds3
  (with-mysql (delete-rows :feeds ["id > 0"]))
  (save-feeds {:entries [{:link link :author "one" :simhash -1}
                         {:link (str link "/two") :author "two" :simhash 2}]} 1)
  (let [feeds (mysql-query ["select * from feeds"])]
    ;; should save
